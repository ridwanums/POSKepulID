...
import com.example.ambarrukmo.api.HttpLoggingInterceptor
...
object RetrofitFactory {
    ...
    private val authInterceptor = Interceptor {
        ...
        val newRequest = original.newBuilder()
            .header("Content-Type", AppConstants.contentType)
            .method(original.method, original.body)
        if (DataManager.getInstance().isLogin == true){
            val token = "Bearer" + DataManager.getInstance().userToken
