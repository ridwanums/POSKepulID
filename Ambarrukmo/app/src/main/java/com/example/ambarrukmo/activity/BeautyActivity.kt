...
import com.example.ambarrukmo.databinding.ActivityBeautyBinding
...
class BeautyActivity : AppCompatActivity() {
    ...
    private fun setData() {
        productViewModel.getCategoryBeautyData().observe(this){
            when (it ){
    ...
    private fun getData(data: MerchantCategory) {
        binding.recyclerViewBeauty.adapter = CategoryAdapter(data)
