...
import com.example.ambarrukmo.adapter.MemberMerchantsAdapter
...
class MemberMerchantsFragment : Fragment() {
    ...
    private fun setTab() {
        val title = "{text}"
        val lisTitle : MutableList<String> = ArrayList()
        for (i in 0..3){
            lisTitle.add(title+" "+i+"")
        }
        binding.viewPager.adapter = MemberMerchantsAdapter(lisTitle, childFragmentManager)
